...
<script language="javascript">
	...
	function insertRows() {
		...
		for (i = 0; i < 4; i++)//每行的3列数据 
		{
			...
			case 0:
				...
				break;
			case 1:
				newCell.innerHTML = "<input name=\"files\"  type=\"file\" size=\"25\">";
				break;
			case 2:
				newCell.innerHTML = "<input name=\"comment\"  type=\"text\" size=\"40\">";
				break;
			case 3:
	...
	function delTableRow(rowNum) {
		...
		if (tbl.rows.length > rowNum) {
			...
			for (i = rowNum; i < tbl.rows.length; i++) {
				...
				tbl.rows[i].cells[3].innerHTML = "<a href='javascript:delTableRow(\""
						+ i
						+ "\")'><img src=${pageContext.request.contextPath }/images/delete.gif width=15 height=14 border=0 style=CURSOR:hand></a>";
			}
		}
	...
	function addFileList() {
		...
		if (check() && overWriteFile("filesTbl", "filesTb2")) {
			document.Form1.action = "dataChart/elecDataChartAction_save.do";
	...
	function check() {
		if (document.getElementById("jctId").value == 0) {
			alert("请选择所属单位！");
			document.getElementById("jctId").focus();
			return false;
		}
	...
	function overWriteFile(tabName1, tabName2) {
		...
		for (i = 1; i < tbl.rows.length; i++) {
			...
			for (j = 1; j < tb2.rows.length; j++) {
				...
				{
					alert("待上传的文件\"" + fileName1
							+ "\"已经存在，不允许上传，或者修改文件名，或者通知管理员删除同名文件后再上传");
					return false;
				} else
					continue;
			}
	...
	function getUrl(projId, filename) {
		var strUrl = "";
		strUrl = "${pageContext.request.contextPath }/UploadFile/Paper/"
				+ projId + "/";
	...
	function changeList() {
		...
		var str = 'dataChart/elecDataChartAction_addList.do?jctID=' + jctId
				+ '&belongTo=' + belongTo;
...
<body onload="init();">
	...
	<form id="Form1" name="Form1"
		action="${pageContext.request.contextPath }/dataChart/elecDataChartAction_save.do"
			...
			<TBODY>
				<tr>
					<td class="ta_01" align="center"
						...
						colspan=3><font face="宋体" size="2"><strong>文件上传管理</strong>
					</font></td>
				</tr>
						...
						<table cellpadding="0" cellspacing="0" border="0" width="100%">
							<tr>
								...
								<td class="ta_01"><s:select list="#request.jctList"
										...
										onchange="changeList();"></s:select></td>
				...
				<tr>
					...
					<TD width="80%" align="right"><input type="button"
						...
						type="button" name="BT_Submit" value="上传" id="BT_Submit"
	...
	<form id="Form2" name="Form2"
		action="${pageContext.request.contextPath }/datachart/elecFileUploadAction_add.do"
		...
		<table cellspacing="0" cellpadding="1" rules="all" bordercolor="gray"
			...
			<tr
				...
				<td class="ta_01" align="center" width="20%"
					background="../images/tablehead.jpg" height=20>编号</td>
				<td class="ta_01" align="center" width="80%"
					background="../images/tablehead.jpg" height=20>已上传文件</td>
			</tr>
			<%--<s:if
				...
				<s:iterator value="#request.dataChartList" status="st">--%>
					<tr onmouseover="this.style.backgroundColor = 'white'"
						...
						<td class="ta_01" align="center" width="80%"><a href="#"
							onclick="openWindow('${pageContext.request.contextPath }/dataChart/elecDataChartAction_download.do?dataChartID=<s:property value="dataChartID"/>','700','400');"><s:property
